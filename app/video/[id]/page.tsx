...
import { getVideoById, VIDEOS } from '@/app/lib/videos';
...
export default function VideoPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/transcripts/${video.transcriptFile}`)
      .then(res => res.text())
      .then(text => {
        try {
  ...
  if (!video) {
    return (
      <main className="flex min-h-screen flex-col items-center p-6 bg-slate-50">
        <h1 className="text-2xl font-bold mb-4">Video không tìm thấy</h1>
        <Link href="/videos" className="text-blue-600 hover:text-blue-800 font-semibold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex gap-4">
              {videoId > 1 && (
                <Link
                  href={`/video/${videoId - 1}`}
                  className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold text-center hover:bg-slate-800 transition"
                >
                  ← Video trước
                </Link>
              )}
              {videoId < VIDEOS.length && (
                <Link
                  href={`/video/${videoId + 1}`}
                  className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold text-center hover:bg-blue-700 transition"
                >
                  Video sau
                </Link>
              )}
